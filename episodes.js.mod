...
async function getMeta(id) {
  ...
  const baseId = cleanId; // **MODIFICA IMPORTANTE: Non rimuovere l'anno dall'ID**
  const seriesLink = `https://ramaorientalfansub.tv/drama/${baseId}/`; // **URL della serie**
  ...
  try {
    ...
    meta.poster = $('img.wp-post-image').attr('src'); // Recupera il poster dalla pagina della serie
...
async function getEpisodes(seriesLink, $, baseId) { // baseId come parametro
  try {
    ...
    console.error('Errore durante il recupero degli episodi:', error);
    return [];
  }
}
